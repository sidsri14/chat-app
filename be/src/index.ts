    ...
    socket.on('message', (data) => {
        try {
            ...
            if (parsed.type === 'join') {
                ...
                if (allSockets.some((x) => x.socket === socket)) return;

                const roomId = typeof parsed.payload?.roomId === 'string' ? parsed.payload.roomId.trim().slice(0, 50) : '';
                const username = typeof parsed.payload?.username === 'string' ? parsed.payload.username.trim().slice(0, 30) : '';
            ...
            if (parsed.type === 'chat') {
                ...
                const rawMessage = typeof parsed.payload?.message === 'string' ? parsed.payload.message.trim().slice(0, 2000) : '';
